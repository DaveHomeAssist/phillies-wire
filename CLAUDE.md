...
| 010 | P1 | resolved | Audit remediation 2026-08-26 (H-1, M-1, M-2) | A required newsletter delivery failure no longer lets the daily publish report success — the daily run now fails on delivery failure (H-1); the two refresh cron workflows merged into one schedule (M-1) (`db678db07`). The `user_data` wrapping test assertion tolerates CRLF checkouts (M-2) (`3e26190f5`). See CHANGELOG 2026-08-26. |
| 011 | P2 | resolved | Audit remediation 2026-08-26 sweep 2 (M-3) + broken publish workflow | The persisted accuracy source was stale (branch held the June 27 report while the deployed artifact was current) because the commit-back `git add` list omitted `dashboard/accuracy/accuracy.json`. Fix: the path is now in the Persist archive snapshot step and the branch copy was synced to the live 2026-08-26 report (Vol. 1 · No. 148). Prerequisite repair: the H-1 gate in `db678db07` overwrote the `Publish summary` step header, leaving a duplicate `run:` key — publish.yml was invalid YAML and every run after 06:25 UTC failed at 0s with "workflow file issue". Restored the step header; strict unique-key YAML parse clean; all 21 test suites pass. See CHANGELOG 2026-08-26. |
